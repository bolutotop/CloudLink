...
Logger::Logger() {
    ...
    QString fileName = QDateTime::currentDateTime()
                           .toString(QString("yyyyMMdd进程ID为%1").arg(GetCurrentProcessId())) + "_applog.txt";

    // 设置完整文件路径
    QString filePath = customLogDir + "/clinet/" + fileName;

    // 确保日志目录存在
    QDir().mkpath(customLogDir + "/clinet");
...
Logger::~Logger() {
 ...
 void Logger::log(const QString& message, const char* file, int line) {
    ...
    QString timestamp = QDateTime::currentDateTime()
                            .toString("[客户端yyyy-MM-dd hh:mm:ss.zzz]");
